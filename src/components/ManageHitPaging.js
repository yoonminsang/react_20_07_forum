...
const ManageHitPaging = ({ itemsCounting }) => {
  ...
  if (pageId === undefined) {
    ...
    for (
      ...
      pagination.push(
        <li className={i == pageId ? "on" : undefined} key={i}>
          <Link to={`/manage/hit/page/${i}`} className="link_num">
            {i}
          </Link>
        </li>
      );
    ...
    prev =
      pageId > 10 ? (
        <Link
          to={`/manage/hit/page/${startIndex - 10}`}
    ...
    for (
      ...
      pagination.push(
        <li className={i == pageId ? "on" : undefined} key={i}>
          <Link
            to={`/manage/hit/search/type/${params.type}/Keyword/${params.Keyword}/page/${i}`}
            className="link_num"
          >
            {i}
          </Link>
    ...
    prev =
      pageId > 10 ? (
        <Link
          to={`/manage/hit/search/type/${params.type}/Keyword/${
            params.Keyword
          }/page/${startIndex - 10}`}
          className="link_paging"
        >
...
export default ManageHitPaging;
